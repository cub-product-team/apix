 ...
 */
import React, { useEffect, Suspense, lazy, useCallback, useState, useRef, useMemo } from 'react';
...
import { LoadingSpinnerIcon, CloseIcon, CloudUploadIcon, UndoIcon, RedoIcon } from './components/icons';
import StoryboardingInput from './components/storyboarding/StoryboardingInput';
import StoryboardingSummary from './components/storyboarding/StoryboardingSummary';
import StoryboardingScenes from './components/storyboarding/StoryboardingScenes';
...
function App() {
    ...
    const { imageToEdit, closeImageEditor, openImageEditor } = useImageEditor();
    const { loginSettings, isLoggedIn, isLoading: authLoading, currentUser } = useAuth();
    ...
    const handleRefDrop = (e: React.DragEvent<HTMLDivElement>) => {
        ...
        if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
            const files = Array.from(e.dataTransfer.files).filter((file: any) => file.type && file.type.startsWith('image/'));
    ...
    }, [isSearchOpen, isGalleryOpen, isInfoOpen, isHistoryPanelOpen, isImageLayoutModalOpen, isBeforeAfterModalOpen, isAppCoverCreatorModalOpen, isLayerComposerVisible, imageToEdit]);
    ...
    const renderContent = () => {
        // Always show storyboard interface directly
        return (
                ...
                <div className="flex-grow flex flex-row overflow-hidden">
                    <aside className="w-1/3 max-w-sm flex flex-col bg-neutral-900/50 p-4 border-r border-white/10">
                        <div className="flex justify-between items-center mb-4 flex-shrink-0">
                            <div className="flex items-center gap-2">
                                <img src="https://thinkclimax.com/wp-content/themes/climax/tpl/dist/assets/images/logo-climax.png" alt="Climax Creatology" className="h-8 w-8" />
                                <h3 className="base-font font-bold text-xl text-yellow-400">Climax Creatology</h3>
                            </div>
                        </div>
                        <div className="flex-grow flex flex-col overflow-y-auto space-y-4 p-2 pr-6 -mr-4">
                            <StoryboardingInput
                        ...
                        <div className="flex-shrink-0 pt-4 border-t border-white/10">
                            {(() => {
                                if (activeInput === 'text') {
                                    if (scenes.length === 0) {
                                        return (
                                            <button onClick={handleDevelopFromText} className="btn btn-primary btn-sm w-full" disabled={isStoryboardLoading || !scriptText.trim()}>
                                                {isStoryboardLoading ? loadingMessage : t('storyboarding_developScenes')}
                                            </button>
                                        );
                                    } else {
                                        return (
                                            <div className="flex items-center gap-2">
                                                <button onClick={() => { setScriptSummary(null); updateScenesAndHistory([]); }} className="btn btn-secondary btn-sm" disabled={isStoryboardLoading}>
                                                    {t('storyboarding_editInput')}
                                                </button>
                                                <button onClick={handleDevelopFromText} className="btn btn-primary btn-sm flex-grow" disabled={isStoryboardLoading}>
                                                    {isStoryboardLoading ? loadingMessage : t('storyboarding_redevelopScenes')}
                                                </button>
                                            </div>
                                        );
                                    }
                                ...
                                if (!scriptSummary) {
                                    return (
                                        <button onClick={handleInitialScriptGeneration} className="btn btn-primary btn-sm w-full" disabled={isStoryboardLoading}>
                                            {isStoryboardLoading ? loadingMessage : t('storyboarding_idea_submit')}
                                        </button>
                                    );
                                }
                                if (scenes.length === 0) {
                                    return (
                                        <div className="flex items-center gap-2">
                                            <button onClick={handleGenerateScriptSummary} className="btn btn-secondary btn-sm" disabled={isStoryboardLoading}>
                                                {t('storyboarding_regenerateScript')}
                                            </button>
                                            <button onClick={handleDevelopScenesFromSummary} className="btn btn-primary btn-sm flex-grow" disabled={isStoryboardLoading}>
                                                {isStoryboardLoading && loadingMessage === t('storyboarding_developing_scenes') ? loadingMessage : t('storyboarding_developScenes')}
                                            </button>
                                        </div>
                                    );
                                }
                                return (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button onClick={handleDevelopScenesFromSummary} className="btn btn-primary btn-sm flex-grow" disabled={isStoryboardLoading}>
                                            {isStoryboardLoading && loadingMessage === t('storyboarding_developing_scenes') ? loadingMessage : t('storyboarding_redevelopScenes')}
                                        </button>
                                    </div>
                                );
                            })()}
                    ...
                    <main className="flex-1 flex flex-col p-4 overflow-hidden bg-neutral-800/30">
                        <div className="flex-shrink-0 pb-4 mb-4 border-b border-white/10 flex items-center justify-end gap-2">
                            ...
                            <button onClick={() => importInputRef.current?.click()} className="btn btn-secondary btn-sm" disabled={isStoryboardLoading}>
                                {t('storyboarding_import')}
                            </button>
                            <button onClick={handleExport} className="btn btn-secondary btn-sm" disabled={isStoryboardLoading || (scenes.length === 0 && !scriptSummary)} title={(scenes.length === 0 && !scriptSummary) ? t('storyboarding_export_disabled') : ''}>
                                {t('storyboarding_export')}
                            </button>
                            <button onClick={handleNew} className="btn btn-secondary btn-sm" disabled={isStoryboardLoading}>
                                {t('storyboarding_new')}
                            </button>
                            <button onClick={handleGenerateAll} className="btn btn-secondary btn-sm" disabled={isStoryboardLoading || scenes.length === 0 || scenes.some(s => s.startFrame.status === 'pending' || s.endFrame.status === 'pending')}>
                                {t('storyboarding_generateAll')}
                            </button>
                            <button onClick={handleDownloadAll} className="btn btn-secondary btn-sm" disabled={isStoryboardLoading || scenes.every(s => !s.startFrame.imageUrl && !s.endFrame.imageUrl)}>
    ...
    return (
        <main className="text-neutral-200 min-h-screen w-full relative">
            ...
            {/* AppToolbar hidden for fullscreen storyboard */}

            <div className="relative z-10 w-full h-screen flex flex-row items-center justify-center">
